...
function printUrlIfReady() {
    if (publicUrl && serverStarted) {
        console.log('=============================================================');
        console.log('📱 ENLACE PÚBLICO LISTO PARA COMPARTIR CON TUS COLEGAS:');
        console.log(`👉  \x1b[32m\x1b[1m${publicUrl}\x1b[0m`);
        console.log('=============================================================\n');
    }
}

// 1. Iniciar servidor usando el comando start (los builds previos se ejecutan en package.json)
const devProcess = spawn('pnpm', ['start'], {
...
devProcess.stdout.on('data', (data) => {
    ...
    if (output.toLowerCase().includes(`puerto 3000`) || output.toLowerCase().includes('corriendo en puerto')) {
        serverStarted = true;
        printUrlIfReady();
    }
});
